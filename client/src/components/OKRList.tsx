...
export const OkrList = ({ okrs, onEdit }: OkrListProps) => {
  ...
  return (
    <div className="relative space-y-12 before:absolute before:-left-px before:top-4 before:bottom-4 before:w-0.5 before:bg-linear-to-b before:from-slate-100 before:via-indigo-100 before:to-slate-100 ml-4 md:ml-0">
      {okrs.map((okr: OKRType, index: number) => {
        const completedKRs = okr.keyResults.filter((kr) => Number(kr.progress) === 100).length;
