...
interface KeyResultProps {
    ...
    index: number;
    onClick: (kr: KeyResultType) => void;
}


const KeyResult = ({keyResult, onClick}: KeyResultProps) => {
    const isFinished = keyResult.progress === keyResult.target;

    return (
        <div
            onClick={() => onClick(keyResult)}
            className={`cursor-pointer group flex items-center gap-4 p-4 rounded-2xl border transition-all duration-300 ${
                isFinished
...
export function KeyResultList({keyResults, objectiveId, onKeyResultClick}: KeyResultListProps) {
            ...
            {keyResults.length > 0 ? (
                keyResults.map((keyResult: KeyResultType, index: number) => (
                    <KeyResult key={keyResult.id} keyResult={keyResult} index={index}
                               onClick={(kr) => onKeyResultClick(kr, objectiveId)}/>
                ))
            ) : (
